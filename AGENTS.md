...
- CI-equivalent focused checks: `bin/rubocop`, `bin/bundler-audit`, `bin/brakeman --quiet --no-pager --exit-on-warn --exit-on-error`, `bin/rails test`, and `env RAILS_ENV=test bin/rails db:seed:replant`.
- GitHub Actions test job uses PostgreSQL on localhost and runs `RAILS_ENV=test DATABASE_URL=postgres://postgres:postgres@localhost:5432 bin/rails db:test:prepare test`.
- Run one Minitest file with `bin/rails test test/path/to/file_test.rb`; add `:line` for a single test case.
...
## Testing Notes
- Test framework is Rails Minitest (`test/test_helper.rb`), with `fixtures :all` and parallel workers enabled.
- There are currently no app-specific test files under `test/**/*_test.rb`; add focused Minitest coverage when introducing behavior.
